...
def extract_datetime_from_filename(fname):
    # 1. 新格式：14位数字开头，如 20250403215812_1.jpg
    ...
    # 2. 旧格式：日期格式开头，如 2012-01-24_00001.jpg
...
class SteamDownloaderApp:
    ...
    def make_widgets(self):
        ...
        ttk.Label(frame, text="Steam ID:").grid(row=0, column=0, sticky="e", pady=3)
        ttk.Entry(frame, textvariable=self.steam_id, width=40).grid(row=0, column=1, columnspan=2, sticky="w", pady=3)

        ttk.Label(frame, text="Download Dir:").grid(row=1, column=0, sticky="e", pady=3)
        ttk.Entry(frame, textvariable=self.download_dir, width=30).grid(row=1, column=1, sticky="w", pady=3)
        ttk.Button(frame, text="Browse", command=self.select_download_dir).grid(row=1, column=2, pady=3)

        ttk.Label(frame, text="Chrome User Data:").grid(row=2, column=0, sticky="e", pady=3)
        ttk.Entry(frame, textvariable=self.chrome_dir, width=30).grid(row=2, column=1, sticky="w", pady=3)
        ttk.Button(frame, text="Browse", command=self.select_chrome_dir).grid(row=2, column=2, pady=3)

        ttk.Label(frame, text="App ID (optional):").grid(row=3, column=0, sticky="e", pady=3)
        appid_frame = ttk.Frame(frame)
        appid_frame.grid(row=3, column=1, columnspan=2, sticky="w", pady=3)
        ...
        ttk.Label(frame, text="Start Page:").grid(row=4, column=0, sticky="e", pady=3)
        ttk.Entry(frame, textvariable=self.start_page, width=10).grid(row=4, column=1, sticky="w", pady=3)

        ttk.Label(frame, text="End Page:").grid(row=5, column=0, sticky="e", pady=3)
        ttk.Entry(frame, textvariable=self.end_page, width=10).grid(row=5, column=1, sticky="w", pady=3)
