...
def init_chrome(user_data_path=None, firefox_path=None):
    options = Options()
    ...
    print(f"Firefox user profile:{user_data_path}")
...
class SteamDownloaderApp:
    def __init__(self, root):
        ...
        # default_path /usr/bin/firefox
        default_firefox_path = "/usr/bin/firefox"
        if os.path.exists(default_firefox_path):
            self.firefox_path.set(default_firefox_path)
        else:
            self.firefox_path.set("")
    ...
    def make_widgets(self):
        ...
        # Chrome Dir
        ttk.Label(frame, text="Firefox User Data:").grid(row=3, column=1, sticky="e", pady=3, padx=(0, 5))
        ttk.Entry(frame, textvariable=self.chrome_dir).grid(row=3, column=2, sticky="ew", pady=3)
        ttk.Button(frame, text="Browse", command=self.select_chrome_dir, width=10).grid(row=3, column=3, pady=3)

        # Firefox Path
        ttk.Label(frame, text="Firefox Location:").grid(row=2, column=1, sticky="e", pady=3, padx=(0, 5))
        ttk.Entry(frame, textvariable=self.firefox_path).grid(row=2, column=2, sticky="ew", pady=3)
        ttk.Button(frame, text="Browse", command=self.select_firefox_path, width=10).grid(row=2, column=3, pady=3)

        # App ID + Max Processes
        ttk.Label(frame, text="App ID (optional):").grid(row=4, column=1, sticky="e", pady=3, padx=(0, 5))
        idproc_frame = ttk.Frame(frame)
        idproc_frame.grid(row=4, column=2, sticky="w", pady=3, columnspan=2)
        ...
        # Start Page
        ttk.Label(frame, text="Start Page:").grid(row=5, column=1, sticky="e", pady=3, padx=(0, 5))
        ttk.Entry(frame, textvariable=self.start_page, width=10).grid(row=5, column=2, sticky="w", pady=3)
        ...
        ttk.Checkbutton(frame, text="Debug Mode", variable=self.debug).grid(row=4, column=2, sticky="w", padx=(460, 0))
    ...
    # Select Firefox executable path
    def select_firefox_path(self):
        path = filedialog.askopenfilename(title="Firefox Location",)
        if path:
