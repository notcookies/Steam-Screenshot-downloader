...
def fetch_img_urls_concurrently_requests(links, cookies, page, processes, max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        def worker(link):
            ...
            if result is None:
                print(f"[Page {page}] Failed link: {link[:80]}...")
            return result
...
def download_img(page, link, img_url, image_url_query, save_dir, stop_flag, debug, idx):
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                return link
    return link  # 修改：失败时返回 link（字符串）

def threaded_download_imgs(img_links, page, save_dir, stop_flag, debug, max_retries=5, retry_history=None, link_to_idx=None, processes=8, tracking=[]):
...
class SteamDownloaderApp:
    ...
    def run_downloader(self):
        ...
        for page in range(start_page, end_page + 1):
            ...
            img_links = fetch_img_urls_concurrently_requests(links, cookies, page, processes, max_retries=3)
            ...
            while retry_links and attempt < max_retries:
                ...
                retry_links, retry_history, new_success = threaded_download_imgs(
                    ...
                    link_to_idx=link_to_idx, processes=processes, tracking=tracking  # 新增：传递 tracking
                )
        ...
        if tracking:
            ...
            # 统计缺失 Content-Disposition
            missing_cd_count = tracking_df['missing_cd'].sum()
            print(f"\n缺失 Content-Disposition 的数量： {missing_cd_count}")

            # 统计每个 app_id 的截图数量
            count_per_app = tracking_df['app_id'].value_counts()
            print("\n每个 app_id 的截图数量：")
            print(count_per_app)

            # 检查 file_name 在每个 app_id 下的唯一性
            duplicate_fnames = tracking_df[tracking_df.duplicated(['app_id', 'file_name'], keep=False)]
            if not duplicate_fnames.empty:
                print("\n每个 app_id 下重复的 file_name：")
                print(duplicate_fnames.to_string(index=False))
            else:
                print("\n每个 app_id 下 file_name 均唯一。")
