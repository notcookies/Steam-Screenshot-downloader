...
# def get_img_url_from_html(link, cookies, ua, lang):
#     try:
#         headers = {
#             "User-Agent": ua,
#             "Accept": (
...
#     batches = [
#         indexed_links[i:i + batch_size]
#         for i in range(0, total_links, batch_size)
#     ]

#     for batch_idx, batch_links in enumerate(batches, start=1):
#         batch_total = len(batch_links)

#         print(f"[Page {page}] Processing batch {batch_idx}/{len(batches)} ({batch_total} links)")

#         for attempt in range(max_retries):

#             def worker(index_link_tuple):
#                 index, link = index_link_tuple
#                 result = get_img_url_from_html(link, cookies, ua, lang)
#                 if result:
...
#                 futures = {
#                     executor.submit(worker, indexed_link): indexed_link
#                     for indexed_link in batch_links
#                 }
...
#             print(
#                 f"[Page {page}] Batch {batch_idx}: "
#                 f"Got {success_count}/{batch_total} image links."
#             )

#             if success_count == batch_total:
#                 all_results.extend(results)
#                 break
#             else:
#                 missing = batch_total - success_count
#                 print(
#                     f"[Page {page}] Batch {batch_idx}: "
#                     f"Missing {missing} links, retrying..."
#                 )
...
#                     print(
#                         f"[Page {page}] Batch {batch_idx}: "
#                         f"❌ Final attempt failed, using {success_count} links."
#                     )
#                     all_results.extend(results)

#         if batch_idx < len(batches):
#             print(f"[Page {page}] Sleeping {sleep_seconds}s before next batch...\n")
#             time.sleep(sleep_seconds)

#     print(f"[Page {page}] ✅ Retrieved {len(all_results)}/{total_links} original image links.\n")
#     return all_results
...
def get_img_url_from_html(link, tab):
    try:
        ...
        tag = soup.select_one(
            'a[href*="//images.steamusercontent.com/ugc/"]'
        )

        if tag:
            link = urlunparse(
                urlparse(tag['href'])._replace(query='')
            )
...
def fetch_img_urls_concurrently_requests(
    ...
    for current, (index, link) in enumerate(indexed_links, start=1):

        print(
            f"[Page {page}] Processing "
            f"{current}/{total_links}"
        )
        ...
        for attempt in range(max_retries):
            ...
            if result:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"[Page {page}] ❌ Failed: {link}"
                    )


    print(
        f"[Page {page}] ✅ Retrieved "
        f"{len(all_results)}/{total_links} original image links.\n"
    )
...
class SteamDownloaderApp:
    def __init__(self, root):
        ...
        self.root = root
        root.title("Steam Screenshot Downloader V2.9.1")
